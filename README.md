...
1. Edit the **Provinator** file [blockchain/truffle.js](/blockchain/truffle.js) so the default _from address_ is the [MetaMask](https://metamask.io/) address from above (0x9ef3569b5dc377eb66cd0895af35b36efec94a88)
2. Change to the **Provinator** directory [blockchain/contracts](/blockchain/contracts), and type `truffle migrate --network rinkeby`
...
1. Run the [IPFS](https://ipfs.io/) daemon: `ipfs daemon`
2. From the **Provinator** home directory, type `ipfs add -r build`
3. Note the [IPFS](https://ipfs.io/) hash of the [build](build) directory

Now load **Provinator** in the [MetaMask](https://metamask.io/) enabled browser, which should still be pointing at [rinkeby](https://www.rinkeby.io):

1. Load the **Provinator** frontend by using the hash of the [build](build) directory generated by `ipfs add` above. For example, imagine the hash is QmZu1xECoQ9VvBA4HzTLQzdmgjwM6uY99Qt7bFmuPJuLqK, then the URL would be [https://gateway.ipfs.io/ipfs/QmZu1xECoQ9VvBA4HzTLQzdmgjwM6uY99Qt7bFmuPJuLqK](https://gateway.ipfs.io/ipfs/QmZu1xECoQ9VvBA4HzTLQzdmgjwM6uY99Qt7bFmuPJuLqK)
2. Use the links within **Provinator** to create a digital media resource and subsequently, get details about that resource
...
## Contributinghttps://gateway.ipfs.io/ipfs/QmdMTucY6CfpH8Yx5orCyq2enYgyKXiqo8hKimozNDwfp6

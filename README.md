...
Export your [MetaMask](https://metamask.io/) account key to [geth](https://github.com/ethereum/go-ethereum):

1. From [MetaMask](https://metamask.io/), export the private key of the newly funded account to a file called _newKey.prv_
2. Run `geth --rinkeby account import newKey.prv`. Give it a passphrase
3. Startup a [geth](https://github.com/ethereum/go-ethereum) console: `geth attach rpc:http://localhost:8545 console`
4. `eth.accounts` will show the newly imported account (and any others that might have been created beforehand)
5. Suppose the imported account has the address _0x9ef3569b5dc377eb66cd0895af35b36efec94a88_. If your [geth](https://github.com/ethereum/go-ethereum) client is up to date with the latest blockchain transactions, then `eth.getBalance("0x9ef3569b5dc377eb66cd0895af35b36efec94a88")` will show its balance
6. Unlock the imported account: `personal.unlockAccount("0x9ef3569b5dc377eb66cd0895af35b36efec94a88")`

Now use [Truffle](https://github.com/trufflesuite/truffle) to publish the **Provenator** smart contracts to [rinkeby](https://www.rinkeby.io):

1. Edit the **Provenator** file [blockchain/truffle.js](/blockchain/truffle.js) so the default _from address_ is the [MetaMask](https://metamask.io/) address from above (0x9ef3569b5dc377eb66cd0895af35b36efec94a88)
2. Change to the **Provenator** directory [blockchain/contracts](/blockchain/contracts), and type `truffle migrate --network rinkeby`
3. Once `truffle` has returned, you can use [Etherscan](https://rinkeby.etherscan.io/) to check the contract addresses

Create the web application:

1. Edit the **Provenator** source file [/app/utils/contractHandler.jsx](/app/utils/contractHandler.jsx) so that the four static variables `premisObjectContractAddress`, `premisEventContractAddress`, `premisAgentContractAddress` and `premisRightsContractAddress` contain the contract addresses on [rinkeby](https://www.rinkeby.io)
2. Change to the **Provenator** repository's home directory
...
Now load **Provenator** in the [MetaMask](https://metamask.io/) enabled browser, which should still be pointing at [rinkeby](https://www.rinkeby.io):

1. Load the **Provenator** frontend by using the hash of the [build](build) directory generated by `ipfs add` above. For example, imagine the hash is QmdMTucY6CfpH8Yx5orCyq2enYgyKXiqo8hKimozNDwfp6, then the URL would be [https://gateway.ipfs.io/ipfs/QmdMTucY6CfpH8Yx5orCyq2enYgyKXiqo8hKimozNDwfp6](https://gateway.ipfs.io/ipfs/QmdMTucY6CfpH8Yx5orCyq2enYgyKXiqo8hKimozNDwfp6)

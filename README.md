...
Once you have all the prerequisites, type `npm install` in the **Provinator** repository's home directory. That should install everything listed in [package.json](/package.json), which form the components of the REACT-based web frontend to this application.

Now, publish the contracts to your local blockchain (via [Ganache](https://github.com/trufflesuite/ganache)):

1. Change directory to the [Ganache](https://github.com/trufflesuite/ganache) repository, then start [Ganache](https://github.com/trufflesuite/ganache) by typing `npm start`.
2. Ensure [Ganache](https://github.com/trufflesuite/ganache) is running on [http://localhost:8545](http://localhost:8545) (you may need to change its settings to do so).
3. Within the **Provinator** repository, change to the directory [/blockchain/contracts](/blockchain/contracts), and type `truffle migrate`.
4. Within the **Provinator** repository, edit the file [contractHandler.jsx](/app/utils/contractHandler.jsx) so that the four static variables `premisObjectContractAddress`, `premisEventContractAddress`, `premisAgentContractAddress` and `premisRightsContractAddress` contain the addresses generated by `truffle migrate`, above. e.g
...
1. In the **Provinator** repository's home directory, build the REACT frontend by typing `npm run copy && npm run watch`.
2. In the **Provinator** repository's home directory, startup an instance of [http-server](https://www.npmjs.com/package/http-server) by typing `npm run start`.
